...
$conn = mysqli_connect("localhost","root","password","rouak_cnrps")or die("Connection was not established");
...
function insertPost(){
	if(isset($_POST['sub'])){
		...
		if(strlen($content) > 250){
			echo "<script>alert('رجاءامنشورك يتجاوز 250كلمة!')</script>";
			...
			if(strlen($upload_image) >= 1  && strlen($content) >= 1){
				...
				if($run){
					echo "<script>alert('تم تحديث منشورك الان!')</script>";
					echo "<script>window.open('home.php', '_self')</script>";
				...
				if($upload_image =='' && $content == ''){
					...
					if($content==''){
						...
						if($run){
							echo "<script>alert('تم تحديث منشورك!')</script>";
							echo "<script>window.open('home.php', '_self')</script>";
...
function get_posts(){
	...
	$run_posts = mysqli_query($conn, $get_posts) or die(mysqli_error($conn));
				...
				<div id='posts' class='col-sm-6'>
					<div class='row'>
						...
						<div class='col-sm-6'>
							...
							color #424141;' href='user_profile.php?u_id=$user_id'>$user_name</a></h3>
							<h4><small style='color:black;'>تم تحديث المنشور في 
					...
					<button class='btn btn-info'>تعليق</button></a><br>
				</div>
				<div class='col-sm-3'>
				</div>
			</div><br>
				...
				<div id='posts' class='col-sm-6'>
					<div class='row'>
						...
						<div class='col-sm-6'>
							...
							<h4><small style='color:black;'>تم التحديث في <strong>$post_date</strong></small></h4>
						</div>
						<div class='col-sm-4'>
						</div>
					</div>
				...
				<div id='posts' class='col-sm-6'>
					<div class='row'>
						...
						href='user_profile.php?u_id=$user_id'>$user_name</a></h3>
							<h4><small style='color:black;'>تم تحديث المنشور في <strong>$post_date</strong></small></h4>
						</div>
						<div class='col-sm-4'>
						</div>
					</div>
...
function single_post(){
					...
					<div id='posts' class='col-sm-6'>
						<div class='row'>
							...
							<div class='col-sm-6'>
								...
								cursor:pointer;color #424141;' href='user_profile.php?u_id=$user_id'>$user_name</a></h3>
								<h4><small style='color:black;'>تم التحديث في <strong>$post_date</strong></small></h4>
							</div>
							<div class='col-sm-4'>
							</div>
					...
					<div id='posts' class='col-sm-6'>
						<div class='row'>
							...
							<div class='col-sm-6'>
								<h3><a style='text-decoration:none; cursor:pointer;color #3897f0;
								' href='user_profile.php?u_id=$user_id'>$user_name</a></h3>
								<h4><small style='color:black;'>تم التحديث في <strong>$post_date</strong>
								</small></h4>
							</div>
							<div class='col-sm-4'>
							</div>
	...
	width='100px' height='100px'></p>
			...
			class='pb-cmnt-textarea'  name='comment'></textarea>			
			 <button class='btn btn-info pull-right' name='replay'>
			تعيقك</button>
			</form>
			</div>
			</div>
			...
			if(isset($_POST['replay'])){
				...
				if($comment== '' ){
					echo "<script>alert('ادخل تعليقك')</script>";
					...
					$run = mysqli_query($conn,$insert);
					echo "<script>alert(' تم اضافة تعليقك')</script>";
...
function search_user(){
	...
	if(isset($_GET['search_user_btn'])){
		...
		f_name like '%$search_query%' or
		l_name like '%$search_query%' or 
